...
const NavBar = () => {
    ...
    return (
        <div className='px-4 flex justify-between items-center w-full h-20 bg-black text-white fixed'>
            <div >
            ...
            {nav && <ul className='flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 text-gray-500'>
                {links.map(({ link, id }) => (
                    <li key={link} className='px-4 cursor-pointer capitalize py-6 text-4xl hover:scale-105 duration-200'>{link}</li>
                ))}
            </ul>}
